...
import {getDayChart} from '../../utils/utools'
let vmt;
let data0;
export default {
  data() {
  ...
  methods: {
    ...
    checkHaveStock() {
        ...
        //    vm.childrenData();
           vmt.getterDayChart(vmt.stockCode);
        }
    },
   //创建第一个charts   
   createdCharts1() {
   ...
   //获取数据
   getterDayChart(stockCode) {
       
       getDayChart(stockCode.toLowerCase(),function (data) {
           let kLine = data.data;
           let rawData = [];
   ...
   startInvData(stockCode) {
       vmt.updateCharts = self.setInterval(function() {
          getDayChart(stockCode.toLowerCase(),function (data) {
           ...
           option.series[3].data =  calculateMA(data0,15);
           option.series[4].data =  calculateMA(data0,20);
           option.series[5].data = getNumbers(data0.values);
